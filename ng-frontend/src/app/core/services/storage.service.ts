...
export class StorageService {
  ...
  private pendingPromises: Map<string, (value: any) => void> = new Map();
  ...
  constructor() {
    ...
    this.socket.onmessage = (event) => {
      ...
      if (message.uuid && this.pendingPromises.has(message.uuid)) {
        const resolve = this.pendingPromises.get(message.uuid);
        this.pendingPromises.delete(message.uuid);
  ...
  // WebSocketMessage
  public Communicate(webSocketMsg: WebSocketMessage): Promise<any> {
    // validate with uuid
    return new Promise((resolve, reject) => {
      this.ensureSocketConnection(() => {
        this.pendingPromises.set(webSocketMsg.uuid, resolve);
        this.socket.send(JSON.stringify(webSocketMsg));
      });
    });
  }
